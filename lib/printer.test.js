...
describe('printer functions', () => {
  ...
  describe('printInvalidField', () => {
    it('should print correct message', () => {
      printInvalidField(USERS, 'abc');
      expect(console.log).toHaveBeenCalledWith(
        `error: "abc" cannot be found in "${USERS}"`
      );
    });
  });

  describe('printTableMissing', () => {
    it('should print correct message', () => {
      printTableMissing();
      expect(console.log).toHaveBeenCalledWith(
        `error: please specify a table to search`
      );
    });
  });
});
